...
from . import utils
from .models import deeplab_resnet, unet
...
class InferenceOutlines():
    ...
    def __init__(self, outlinesWeightsFile, outlinesModel='deeplab_resnet', imgHeight=288, imgWidth=512):
        ...
        Args:
            ...
            outlinesModel (str): Which model to use. Can be one of ["unet", "deeplab_resnet"]
        ...
        # Create Model
        if outlinesModel == 'unet':
            print('Creating Unet model for outlines and loading checkpoint')
            self.model = unet.Unet(num_classes=3)
        elif outlinesModel == 'deeplab_resnet':
            print('Creating Deeplabv3-Resnet model for outlines and loading checkpoint')
            self.model = deeplab_resnet.DeepLabv3_plus(n_classes=3,
                                                       os=8,
                                                       nInputChannels=3,
                                                       pretrained=False,
                                                       _print=False)
...
class InferenceNormals():
    ...
    def __init__(self, normalsWeightsFile, normalsModel='deeplab_resnet', imgHeight=288, imgWidth=512):
        ...
        Args:
            ...
            normalsModel (str): Which model to use. Can be one of ["unet", "deeplab_resnet"]
            imgHeight (int): The height to which images should be resized to before passing to the model.
            imgWidth (int): The width to which images should be resized to before passing to the model.
        '''

        # Create Model
        if normalsModel == 'unet':
            print('Creating Unet model for surface normals and loading checkpoint')
            self.model = unet.Unet(num_classes=3)
        elif normalsModel == 'deeplab_resnet':
...
class InferenceMasks():
    ...
    def __init__(self, masksWeightsFile, masksModel='drn', imgHeight=288, imgWidth=512):
        ...
        Args:
            ...
            masksModel (str): Which model to use. Can be one of ["unet", "deeplab_resnet"]
            imgHeight (int): The height to which images should be resized to before passing to the model.
            imgWidth (int): The width to which images should be resized to before passing to the model.
        '''

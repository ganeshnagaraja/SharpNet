...
class DepthToDepthCompletion(object):
    ...
    Attributes:
        ...
        filter_sigmaColor: Default: 100. Param of bilateral filer for output depth.
        ...
        filter_sigmaSpace: Default: 100. Param of bilateral filer for output depth.
    ...
    FOLDER_MAP = {
        ...
        #     'postfix': '-output-depth.exr',
        #     'folder-name': 'filtered-output-depth'
        # },
        # 'filtered-output-point-cloud': {
        #     'postfix': '-output-pointcloud.ply',
        #     'folder-name': 'filtered-output-point-cloud'
        # },
        'masks': {
            'postfix': '-mask.png',
    ...
    def store_depth_completion_outputs(self, root_dir, files_prefix, min_depth=0.0, max_depth=1.5):
        ...
        Returns:
            float32: Error on output depth
            float32: Error on filtered output depth
        """
        ...
        # filtered_output_depth_dir = os.path.join(root_dir, self.FOLDER_MAP['filtered-output-depth']['folder-name'])
        # filtered_output_ptcloud_dir = os.path.join(root_dir,
        #                                            self.FOLDER_MAP['filtered-output-point-cloud']['folder-name'])
        ...
        # Save filtered output depth and point cloud
        # output_depth_filename = '{:09d}'.format(files_prefix) + self.FOLDER_MAP['filtered-output-depth']['postfix']
        # output_depth_filename = os.path.join(filtered_output_depth_dir, output_depth_filename)
        # utils.exr_saver(output_depth_filename, self.filtered_output_depth, ndim=3)

        # output_ptcloud_filename = ('{:09d}'.format(files_prefix) +
        #                            self.FOLDER_MAP['filtered-output-point-cloud']['postfix'])
        # output_ptcloud_filename = os.path.join(filtered_output_ptcloud_dir, output_ptcloud_filename)
        # utils.write_point_cloud(output_ptcloud_filename, self.surface_normals_rgb, self.filtered_output_depth, self.fx,
        #                         self.fy, self.cx, self.cy)

        # Store Masks
        mask_filename = '{:09d}'.format(files_prefix) + self.FOLDER_MAP['masks']['postfix']
        mask_filename = os.path.join(masks_dir, mask_filename)
        imageio.imwrite(mask_filename, self.mask)
        ...
        output_depth_rgb = utils.depth2rgb(self.output_depth,
                                           ...
                                           max_depth=max_depth,
                                           color_mode=COLOR_MAP)
        filtered_output_depth_rgb = utils.depth2rgb(self.filtered_output_depth,
                                                    min_depth=min_depth,
                                                    max_depth=max_depth,
                                                    color_mode=COLOR_MAP)
        ...
        mask_rgb[self.mask > 0, 0] = 255
        mask_rgb[self.outlines_rgb[:, :, 1] > 0, 1] = 255
        # common_area = np.logical_and((self.mask > 0), (self.outlines_rgb[:, :, 1] > 0))
        # mask_rgb[common_area, :] = 0
        # mask_rgb[common_area, 2] = 255
        ...
        grid_image1 = np.concatenate(
            (self.input_image, self.surface_normals_rgb, self.outlines_rgb, self.occlusion_weight_rgb), 1)
        grid_image2 = np.concatenate(
            (masked_img, orig_input_depth_rgb, input_depth_rgb, output_depth_rgb), 1)
    ...
    def _modify_depth_delete_masks(self, rgb_image, input_depth):
        # Run inference to get mask of transparent objects
        self.mask = self.inferenceMasks.runOnNumpyImage(rgb_image)
        self.mask = cv2.resize(self.mask, (self.outputImgWidth, self.outputImgHeight), interpolation=cv2.INTER_NEAREST)
        ...
        modified_input_depth[self.mask == 255] = 0.0
    ...
    def _modify_depth_delete_masks_normals(self, rgb_image, input_depth):
        ...
        # Run inference to get mask of transparent objects
        mask = self.inferenceMasks.runOnNumpyImage(surface_normals_rgb)
        self.mask = cv2.resize(mask, (self.outputImgWidth, self.outputImgHeight), interpolation=cv2.INTER_NEAREST)
    ...
    def depth_completion(self,
        ...
        Args:
            ...
            mode_modify_input_depth (str, optional): What method to use to modify input depth (see desc above).
                                                     ...
                                                     Defaults to None.
            percentageHoles (float): Only used if mode is "random_holes".
                                     The percentage of pixels from input depth to be randomly removed (made 0.0)
                                     Range of expected values: [0.0, 1.0]
            resize_factor (int): Only used if mode is "random_holes".
                                 The number by which output image height and width will be divided before deleting
                                 pixels. This has the effect of creating larger holes. Note that output image size
                                 is defined upon init of this class.
                                 Eg: If the resize factor is 4, each deleted pixel will create a hole that is 4 pixels
                                 wide in height and width.
        ...
        # modify input depth
        if mode_modify_input_depth is None:
            self.input_depth = self.orig_input_depth
        elif mode_modify_input_depth == 'random_holes':
            self.input_depth = self._modify_depth_delete_random_pixels(self.orig_input_depth,
                                                                       percentageHoles,
            ...
            self.input_depth = self._modify_depth_delete_masks(self.input_image, self.orig_input_depth)
        elif mode_modify_input_depth == 'mask_normals':
            if not self.masksWeightsFile:
                raise ValueError('no "masksWeightsFile" has been specified')

            self.input_depth = self._modify_depth_delete_masks_normals(self.input_image, self.orig_input_depth)
        else:
            raise NotImplementedError('Mode of depth modification can only be None, "random_holes"' +
                                      'or "mask", "mask_normals"')

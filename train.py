...
def train_epoch(train_loader,
    ...
    for iter_i, _ in enumerate(tqdm(train_loader)):
        ...
        if config.train.verbose:
            print("\nepoch: " + str(epoch) + " | iter: {}/{} ".format(iter_i, iter_per_epoch) + "| Train Loss: " +
                  str(float(iter_loss)) + '\n')
        train_writer.add_scalar("train_loss", train_loss_meter.value()[0], int(epoch) * iter_per_epoch + iter_i)

        write_loss_components(train_writer, iter_i, epoch, train_size, config.train, depth_loss_meter, iter_depth_loss,
                              normals_loss_meter, iter_normals_loss, boundary_loss_meter, iter_boundary_loss,
                              grad_loss_meter, iter_grad_loss, consensus_loss_meter, iter_consensus_loss)
...
def get_trainval_splits(config):
    ...
    if args.dataset == 'PBRS':
        train_dataset = PBRSDataset(img_list=train_files,
        ...
        val_dataset = PBRSDataset(img_list=val_files,
                                  root_dir=args.root_dir,
                                  transforms=t,
                                  use_depth=True if args.depth else False,
                                  use_boundary=True if args.boundary else False,
        ...
        train_dataset = NYUDataset('nyu_depth_v2_labeled.mat',
                                   split_type='train',
                                   root_dir=args.root_dir,
                                   transforms=t,
                                   use_depth=True,
                                   use_boundary=False,
                                   use_normals=False)
        val_dataset = NYUDataset('nyu_depth_v2_labeled.mat',
                                 split_type='test',
                                 root_dir=args.root_dir,
                                 transforms=t,
                                 use_depth=True,
        ...
        for dataset in config.train.datasetsTrain:
            train_dataset = ClearGraspDataset(dataset.rgb,
                                              dataset.depth,
                                              dataset.normals,
                                              dataset.outlines,
                                              dataset.masks,
        ...
        for dataset in config.train.datasetsVal:
            val_dataset = ClearGraspDataset(dataset.rgb,
                                            dataset.depth,
                                            dataset.normals,
                                            dataset.outlines,
                                            dataset.masks,
...
def main():
    ...
    resnet_dict = {
        k.replace('.', '_img.', 1): v for k, v in resnet50_dict.items() if k.replace('.', '_img.', 1) in model_dict
    ...
    if config.train.pretrained_model is not None:
        ...
        if config.train.depth:
            ...
            pretrained_dict = {
                k: v for k, v in tmp_dict.items() if (k in model_dict and not k.startswith('depth_decoder'))
            }
    ...
    for epoch in range(config.train.max_epoch):
        if config.train.optimizer == 'SGD':
            adjust_learning_rate(float(config.train.learning_rate), config.train.lr_mode,
                                 float(config.train.gradient_step), config.train.max_epoch, optimizer, epoch)
